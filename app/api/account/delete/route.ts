  ...
  try {
    ...
    try {
      ...
      if (userRole === 'merchant') {
        ...
        await db
          .delete(esercenteTaxDetails)
          .where(eq(esercenteTaxDetails.esercenteId, userId));
        ...
        await db
          .delete(riderTaxDetails)
          .where(eq(riderTaxDetails.riderId, userId));

        // Delete disponibilità
        await db
          .delete(disponibilitaRiders)
          .where(eq(disponibilitaRiders.riderId, userId));
      ...
      if (deleteAuthError) {
        console.error(
          '⚠️ Warning: Failed to delete from auth:',
          deleteAuthError
        );
