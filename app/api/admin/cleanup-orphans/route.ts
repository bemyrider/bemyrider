...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🔍 Found ${orphanedProfiles.length} orphaned profiles:`, orphanedProfiles)
    ...
    for (const profile of orphanedProfiles) {
      try {
        console.log(`🗑️ Cleaning up orphaned profile: ${profile.id} (${profile.full_name})`)
        ...
        if (ridersDetailsError) {
          console.error(`Error deleting riders_details for ${profile.id}:`, ridersDetailsError)
        }
        ...
        if (taxDetailsError) {
          console.error(`Error deleting riders_tax_details for ${profile.id}:`, taxDetailsError)
        }
        ...
        if (bookingsError) {
          console.error(`Error deleting bookings for ${profile.id}:`, bookingsError)
        }
        ...
        if (reviewsError) {
          console.error(`Error deleting reviews for ${profile.id}:`, reviewsError)
        }
    ...
    console.log(`✅ Orphan cleanup completed. Cleaned ${cleanedCount} profiles.`)
