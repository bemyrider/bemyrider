...
async function checkAndUpdateOnboardingStatus(stripeAccountId: string, supabase: any) {
  try {
    console.log('🔍 Checking onboarding status for Stripe account:', stripeAccountId)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (checkError && checkError.code !== 'PGRST116') { // PGRST116 = no rows returned
      ...
      return NextResponse.json({ error: 'Failed to check existing rider' }, { status: 500 })
    }

    if (existingRider?.stripe_account_id) {
      console.log('ℹ️ User already has Stripe account:', existingRider.stripe_account_id)
      console.log('🔄 Current onboarding status:', existingRider.stripe_onboarding_complete)
      
      // Verifica e aggiorna lo stato di onboarding
      const currentStatus = await checkAndUpdateOnboardingStatus(existingRider.stripe_account_id, supabase)
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const currentStatus = await checkAndUpdateOnboardingStatus(riderDetails.stripe_account_id, supabase)
