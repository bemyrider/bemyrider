...
import { ArrowLeft, Calendar, Clock, MapPin, User, MessageSquare, CheckCircle, XCircle, AlertCircle, Edit, Trash2 } from 'lucide-react'
...
export default function MerchantRequestsPage() {
  ...
  const fetchServiceRequests = useCallback(async () => {
    ...
    try {
      ...
      const transformedRequests = (requestsData || []).map(request => {
        const rider = Array.isArray(request.rider) ? request.rider[0] : request.rider
        return {
          ...request,
          rider: {
            ...rider,
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'accepted':
        return <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">Accettata</Badge>
      case 'rejected':
        return <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200">Rifiutata</Badge>
      case 'expired':
        return <Badge variant="outline" className="bg-gray-50 text-gray-700 border-gray-200">Scaduta</Badge>
      default:
  ...
  const handleDeleteRequest = async (requestId: string) => {
    ...
    if (!confirm('Sei sicuro di voler eliminare questa richiesta di servizio?')) {
      return
    }
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold text-gray-900">Gestisci Richieste di Servizio</h1>
          <p className="text-gray-600">Visualizza e gestisci tutte le richieste di servizio inviate ai rider</p>
        </div>
        ...
        {serviceRequests.length === 0 ? (
          <Card>
            <CardContent className="text-center p-8">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">Nessuna Richiesta di Servizio</h3>
              <p className="text-gray-600 mb-4">Non hai ancora inviato richieste di servizio ai rider.</p>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-start space-x-4 flex-1">
                      ...
                      <div className="w-12 h-12 rounded-full overflow-hidden bg-gray-100 flex items-center justify-center flex-shrink-0">
                        {request.rider.riders_details?.profile_picture_url || request.rider.avatar_url ? (
                          <Image 
                            src={request.rider.riders_details?.profile_picture_url || request.rider.avatar_url || ''} 
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <span>{formatTime(request.start_time)} - {request.duration_hours}h di servizio</span>
                          </div>
                          ...
                          <div className="flex items-center space-x-2">
                            <span className="font-medium">
                              {formatCurrency((request.rider.riders_details?.hourly_rate || 0) * request.duration_hours)}
                            </span>
                          </div>
                        </div>
                        ...
                        {request.rider_response && (
                          <div className="mt-3 p-3 bg-blue-50 rounded-lg">
                            <div className="flex items-start space-x-2">
                              ...
                              <div>
                                <p className="text-sm font-medium text-blue-900 mb-1">Risposta del Rider:</p>
                                <p className="text-sm text-blue-700">{request.rider_response}</p>
                              </div>
                            </div>
                          </div>
                        )}
