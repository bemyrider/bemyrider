...
export default function RiderBookingPage() {
  ...
  // API retry hook per gestire errori di connessione
  const {
    ...
    onError: error => {
      ...
      toast({
        ...
        description:
          "Impossible completare l'operazione. Verifica la tua connessione internet.",
      });
    },
  });
  ...
  const fetchRiderProfile = useCallback(async () => {
    const result = await executeWithRetry(async () => {
      ...
      if (profileError) {
        ...
        throw new Error('Rider non trovato');
      }
      ...
      if (availabilityError) {
        ...
        // Non bloccare il caricamento per errori di disponibilità
      } else {
    ...
    setLoading(false);
  }, [riderId, executeWithRetry]);

  useEffect(() => {
    fetchRiderProfile();
  }, [riderId, fetchRiderProfile]);
  ...
  }, [startDate, startTime, duration, availability, validateAvailability]);
        ...
        <div className='text-center'>
          ...
          <p className='mt-4 text-gray-600'>
            {apiLoading
              ...
              : 'Caricamento profilo rider...'}
          </p>
          {apiError && (
            <p className='mt-2 text-sm text-red-500'>
              Problema di connessione. Riprova tra qualche memento.
            </p>
          )}
        </div>
      </div>
