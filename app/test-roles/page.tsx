    ...
    <div className="max-w-6xl mx-auto p-8">
            ...
            {userInfo ? (
              <div className="space-y-2">
                ...
                <p><strong>Role:</strong> <span className={`px-2 py-1 rounded ${
                  userInfo.profile?.role === 'merchant' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'
                }`}>{userInfo.profile?.role || 'No role'}</span></p>
                <p><strong>Name:</strong> {userInfo.profile?.full_name || 'N/A'}</p>
              </div>
            ) : (
              <p>Not logged in. <a href="/auth/login" className="text-blue-600 underline">Login</a></p>
            )}
          </CardContent>
        </Card>
      ...
      <Card className="mb-6">
        <CardHeader>
          ...
          <CardDescription>Test navigation to different dashboards</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <Button variant="outline" onClick={() => router.push('/debug-user')}>
              Debug User Info
            </Button>
          </div>
        </CardContent>
      ...
      {testResults.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Results from automated protection tests</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <CardContent>
          <div className="text-sm space-y-2">
            ...
            <ul className="list-disc ml-6 space-y-1">
              ...
              <li>Accessing wrong dashboard → automatic redirect to correct one</li>
              <li>Not logged in → redirect to /auth/login</li>
              <li>Login → automatic redirect based on user role</li>
            </ul>
            
            <p className="mt-4"><strong>🚫 Security violations:</strong></p>
            <ul className="list-disc ml-6 space-y-1">
