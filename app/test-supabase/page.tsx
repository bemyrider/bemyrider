  ...
  useEffect(() => {
    const testQueries = async () => {
      try {
        ...
        const { data: esercentiData, error: esercentiError } = await supabase
          .rpc('get_all_esercenti')
        ...
        if (authError || !user) {
          ...
          const { data: userProfileArray, error: upError } = await supabase
            .rpc('get_profile_by_id', { profile_id: user.id })
            
          userProfile = userProfileArray && userProfileArray.length > 0 ? userProfileArray[0] : null
          userProfileError = upError
            
          if (userProfileError) {
            console.error('❌ User profile RPC query failed:', userProfileError)
        ...
        setResults([
          ...
          { test: 'User Profile RPC', data: userProfile, error: userProfileError }
        ])
