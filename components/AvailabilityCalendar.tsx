...
export default function AvailabilityCalendar({ riderId, onClose }: AvailabilityCalendarProps) {
    ...
    setAvailability(prev => prev.map(day => 
      day.day === dayKey 
        ? { 
            ...
            timeSlots: !day.isAvailable ? [{ start_time: '09:00', end_time: '17:00' }] : []
          }
        : day
    ))
  }
  ...
  const updateTimeSlot = (dayKey: string, slotIndex: number, field: 'start_time' | 'end_time', value: string) => {
    setAvailability(prev => prev.map(day => 
      day.day === dayKey 
        ? { 
            ...day, 
  ...
  const saveAvailability = async () => {
    try {
      ...
      setMessage({ type: 'success', text: 'Disponibilità salvata con successo!' })
      setTimeout(() => {
        onClose()
      }, 1500)
                      ...
                      <div className="space-y-2">
                        {dayData.timeSlots.map((slot, slotIndex) => (
                          <div key={slotIndex} className="flex items-center space-x-2 bg-gray-50 p-2 rounded">
                            ...
                            <div className="flex items-center space-x-2">
                              <Label htmlFor={`start-${key}-${slotIndex}`} className="text-sm">Da:</Label>
                              <Input
                                id={`start-${key}-${slotIndex}`}
                                type="time"
                                value={slot.start_time}
                                onChange={(e) => updateTimeSlot(key, slotIndex, 'start_time', e.target.value)}
                                className="w-32"
                              />
                            </div>
                            <div className="flex items-center space-x-2">
                              <Label htmlFor={`end-${key}-${slotIndex}`} className="text-sm">A:</Label>
                              <Input
                                id={`end-${key}-${slotIndex}`}
                                type="time"
                                value={slot.end_time}
                                onChange={(e) => updateTimeSlot(key, slotIndex, 'end_time', e.target.value)}
                                className="w-32"
                              />
                            </div>
                        ...
                        {dayData.timeSlots.length === 0 && (
                          <p className="text-sm text-gray-500 italic">Nessuna fascia oraria impostata</p>
                        )}
                      </div>
                    )}
                  </CardContent>
