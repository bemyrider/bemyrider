...
export async function middleware(req: NextRequest) {
  ...
  if (!isPublicRoute) {
    try {
      ...
      // Verify authenticated user server-side for security
      const {
        data: { user },
        error: userError,
      ...
      if (userError) {
        ...
        if (userError.name === 'AuthSessionMissingError') {
          ...
          // Other auth errors are critical
          console.error('❌ Middleware: Critical auth error:', userError);
          const loginUrl = new URL('/auth/login', req.url);
          loginUrl.searchParams.set('redirectTo', pathname);
          return NextResponse.redirect(loginUrl);
        }
      ...
      // Also verify session for API compatibility
      const {
        data: { session },
        error: sessionError,
