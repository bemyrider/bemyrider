...
You can use either a `***REMOVED***` url connection string or the options to define your database connection properties. Options in the object will override any present in the url. Options will fall back to the same environment variables as psql.

```js
const sql = postgres('***REMOVED***username:password@host:port/database', {
...
Multiple connection strings can be passed to `postgres()` in the form of `postgres('***REMOVED***localhost:5432,localhost:5433', ...)`. This works the same as native the `psql` command. Read more at [multiple host URIs](https://www.postgresql.org/docs/13/libpq-connect.html#LIBPQ-MULTIPLE-HOSTS).
